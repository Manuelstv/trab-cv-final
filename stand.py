...
path = "/home/msnuel/trab-final-cv/animals/zebra2"
...
for file in dirs:
    if file.endswith('jpg'):
        ...
        top_left_corner = ((float(x)-float(w)/2)*im_w, (float(y)-float(h)/2)*im_h)
        bottom_right_corner = ((float(x)+float(w)/2)*im_w, (float(y)+float(h)/2)*im_h)
